...
def evaluate(model, sess, test_batch):
    ...
    test_u_oracle_user_ebd, test_u_mask_num_2nd_user, test_u_mask_num_3rd_item, \
    test_i_1st_user, test_i_2nd_item, test_i_3rd_user, test_i_oracle_item_ebd, test_i_mask_num_2nd_item, test_i_mask_num_3rd_user = test_batch
    ...
    for user_index in tqdm.tqdm(test_user_length):
        ...
        for i_batch_id in range(n_item_batchs):
            ...
            predict_batch = sess.run([model.batch_predicts], feed_dict)  
        ...
        precisions = []
        for k in Ks:
            precisions.append(metrics.precision_at_k(r, k))
        precision.append(precisions)
        recalls = []
        for k in Ks:
            recalls.append(metrics.recall_at_k(r, k,len(user_pos_test)))
        recall.append(recalls)
        # recall.append(metrics.recall_at_k(r, Ks[0], len(user_pos_test)))
        ndcgs = []
        for k in Ks:
            ndcgs.append(metrics.ndcg_at_k(r, k))
            # ndcgs.append(metrics.precision_at_k(r, k))
        ndcg.append(ndcgs)

        hit_ratios = []
        for k in Ks:
            hit_ratios.append(metrics.hit_at_k(r, k))
        hit_ratio.append(hit_ratios)
        # ndcg.append(metrics.ndcg_at_k(r, Ks[0]))
        # hit_ratio.append(metrics.hit_at_k(r, Ks[0]))
    return np.mean(np.array(precision),axis=0), np.mean(np.array(recall),axis=0), np.mean(np.array(ndcg),axis=0), np.mean(
        np.array(hit_ratio),axis=0)
...
def revise_test_neighbors_item_task_downstream(name):
    ...
    with g_rl_revise.as_default():
        with sess3.as_default():
            ...
            all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = model.generate_batch_state_ebd_item_task_downstream(
            ...
            for batch_index in tqdm.tqdm(range(num_batch)):
                ...
                b_target_item, b_k_shot_user, b_second_order_items, b_third_order_users, b_oracle_item_ebd, b_mask_num_second_order_item, b_mask_num_third_order_user = downstream_data.batch_gen_mix_item_task(
                    item_batches, batch_index)

                b_target_item = np.reshape(b_target_item, (-1))
                b_mask_num_second_order_item = np.reshape(b_mask_num_second_order_item, (-1))
                b_target_ori_ebd, b_target_aggre_ebd, b_target_second_aggre_ebd, b_first_neigh_ori_ebd, b_second_neigh_ori_ebd, b_third_neigh_ori_ebd, b_prob1, b_prob2, b_prob3 = \
                    ...
                    all_second_neigh_ori_ebd[
                        batch_index], \
                    all_third_neigh_ori_ebd[
                        batch_index], \
                    all_first_order_reward1[
                ...
                original_reward_2rd = model.produce_original_reward_item_task_downstream(item_batches, num_batch,
                                                                              downstream_data)
                original_reward_3rd = model.produce_original_reward_3rd_item_task_downstream(item_batches, num_batch, downstream_data)
            ...
            # concatnate
            item_input, i_1st_user, i_2nd_item, i_3rd_user, i_oracle_item_ebd, i_mask_num_2nd_item, i_mask_num_3rd_user = item_batches
            ...
            for index in range(len(r_target_item)):
                if index == 0:
                    ...
                    final_r_target_item = np.concatenate([final_r_target_item, r_target_item[index]], 0)
...
def revise_test_neighbors_user_task_downstream(name):
    ...
    with g_rl_revise.as_default():
        with sess3.as_default():
            ...
            all_target_ori_ebd, all_target_aggre_ebd, all_target_second_aggre_ebd, all_first_neigh_ori_ebd, all_second_neigh_ori_ebd, all_third_neigh_ori_ebd, all_first_order_reward1, all_first_and_second_order_reward2, all_former_third_order_reward3 = model.generate_batch_state_ebd_user_task_downstream(
                users_batches, downstream_data)

            r_target_user, r_k_shot_item, r_selected_input_2rd, r_selected_input_3rd, r_oracle_user_ebd = [], [], [], [], []
            for batch_index in tqdm.tqdm(range(num_batch)):
                b_target_user, b_k_shot_item, b_second_order_users, b_third_order_items, b_oracle_user_ebd, b_mask_num_second_order_user, b_mask_num_third_order_item = downstream_data.batch_gen_mix_user_task(
                ...
                original_reward_2rd = model.produce_original_reward_user_task_downstream(users_batches, num_batch,
                                                                              downstream_data)
                original_reward_3rd = model.produce_original_reward_3rd_user_task_downstream(users_batches, num_batch, downstream_data)
            ...
            # concatnate
            user_input, u_1st_item, u_2nd_user, u_3rd_item, u_oracle_user_ebd, u_mask_num_2nd_user, u_mask_num_3rd_item = users_batches
            trancated_user_input = user_input[num_batch * setting.batch_size:setting.num_users,:]
            trancated_user_input = np.reshape(trancated_user_input, (-1))
...
def train_downstream_task_revised(name):
    ...
    with g_rl_revise.as_default():
        with sess3.as_default():
            ...
            pos_item_input, ipos_1st_user, ipos_2nd_item, ipos_3rd_user, ipos_oracle_item_ebd, ipos_mask_num_2nd_item, ipos_mask_num_3rd_user, \
            neg_item_input, ineg_1st_user, ineg_2nd_item, ineg_3rd_user, ineg_oracle_item_ebd, ineg_mask_num_2nd_item, ineg_mask_num_3rd_user = data.get_train_instances_revised(
                train, 0)
            ...
            for epoch in range(setting.epochs):
                ...
                if epoch % 1 == 0:
                    ...
                    t2 = time()
                    print(
                        'Iteration %d [%.1f s]: precision = %s, recall = %s, ndcg = %s, hr = %s, loss = %.4f [%.1f s]'
